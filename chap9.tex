...
L'utilité des macros est donc de faciliter la tâche de l'utilisateur en le dispensant de réencoder plusieurs
fois la même séquence d'instructions. Éventuellement l'utilisateur recevra d'une autre personne un fichier
contenant des macros qu'il pourra exécuter comme des \og boîtes noires\fg. Dans un cadre scolaire, ceci n'est
pas à conseiller car la réalisation personnelle d'une macro par un élève contribue à la compréhension du
domaine étudié. C'est donc une activité d'apprentissage à ne pas négliger, pour autant que la macro en
question soit effectivement susceptible d'être utilisée à plusieurs reprises.
...
Depuis la version 2.0.4, la barre de menus comporte un menu \texttt{Macros}. À l'ouverture d'une session, il
ne comprend que deux items:
                                                    \begin{center}
\includegraphics[width=0.8\linewidth]{macro1.jpg}
\leg{}
\end{center}

L'item \texttt{Charger une macro} suppose que des macros ont déjà été créées et stockées sur l'ordinateur.
...
En résumé: vous pouvez insérer dans une macro toutes les opérations de construction d'une forme libre, ainsi
que les opérations     \texttt{Diviser}, \texttt{Dupliquer un point "sur"},     \texttt{Prolonger},
\texttt{Construire le centre} ainsi que toutes les opérations impliquant une
transformation géométrique.                       Vus pouvez aussi utiliser dans une macro une ou plusieurs macros construites antérieurement.
...
D'autre part, toute macro utilisée lors de la réalisation d'un fichier de formes géométriques ets incorporée à celui-ci lors de la sauvegarde.
...
À titre d'exemple, imaginons une macro qui construit un carré à partir de deux sommets voisins de ce carré. Il sera possible de modifier le carré en tirant sur un de ces sommets mais non en tirant sur un des deux autres sommets.
Le carré ainsi construit par une macro est bien un carré, mais ce n'est pas une forme libre au même titre que celles construites par les méthodes usuelles, à partir du pavé des formes libres.

On voit donc que les objets construits à l'aide de macros sont moins souples que les autres, ils sont modifiés moins facilement. On ne peut pas non plus leur appliquer un mouvement directement: cela serait contradictoire avec leur construction. Par contre les
