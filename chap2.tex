...
%qui apparaissaient \textsc{avant} la version 2.3.6.}.
Le reste de  l'écran constitue la \textit{fenêtre de
dessin} où une \textit{figure} sera construite.\index{Menu}\index{Outil}\index{Operation@Opération}\index{Mouvement}\index{Forme!standard} Les touches
\fbox{\strut+}, \fbox{\strut-} et \fbox{\strut=} permettent de modifier la taille des caractères des textes affichés sur les
différents boutons (mais pas ceux de la barre des menus). Depuis la version 2.5.0, la taille des \textit{étiquettes}
apposées par l'utilisateur sur une figure sont également modifiées.   Ceci permet de faciliter l'utilisation du logiciel
par des personnes malvoyantes.\index{Étiquette}
...
\includegraphics[width=\linewidth]{Pavstandard2.jpg}
...
clic.\end{minipage}}\vfill

\fbox{\begin{minipage}[c]{0.99\linewidth}\small \Prat
...
de la forme sélectionnée. \end{list}\vspace{\baselineskip} \end{minipage}}\vfill
...
vire au magenta et le message affiché devient \og ce point ?\fg. \bigskip\vfill
...
sommet commun.\vspace{\baselineskip}
\end{minipage}
\end{minipage}}\vfill
...
\centerline{\includegraphics[width=0.75\linewidth]{Famtriequi_menu.jpg}}
...
\begin{minipage}[b]{0.2\linewidth}\begin{center}\includegraphics[width=0.3\linewidth]{Petitcercle_TrA.eps}\end{center}
\end{minipage}\hfill
\begin{minipage}{0.3\linewidth} un petit disque\index{Disque} dans lequel on peut inscrire
un triangle équilatéral; \end{minipage}\hfill
\begin{minipage}[b]{0.2\linewidth}\begin{center}\includegraphics[width=0.6\linewidth]{Grandcercle_TrA.eps}\end{center}
\end{minipage}\hfill
\begin{minipage}{0.3\linewidth} un grand disque\index{Disque} dans lequel on peut
...
\begin{minipage}{0.55\linewidth}
\centerline{\includegraphics[width=0.5\linewidth]{Famcar.eps}}
\leg{\label{Famcar.eps}}
\end{minipage} \hfill
\begin{minipage}{0.45\linewidth}
\centerline{\includegraphics[width=0.75\linewidth]{Famcar_menu.jpg}}
...
\centerline{\includeg{0.8\linewidth}{0.7\linewidth}{Fampentag_menu.jpg}}
\vspace{0.3cm}
\end{minipage}
\end{minipage}}
...
Imaginons la situation suivante.                  À partir d'un
des menus B, C, AB ou AC, un enseignant a construit un carré:
c'est  une forme libre (voir le chapitre \ref{chapflib}) qu'il
peut agrandir à volonté. En joignant les milieux de deux côtés,
 il découpe (voir \hyperlink{chapoper}{Decouper})
 deux \og coins\fg\ de ce carré en vue de réaliser une
activité de puzzle: réassembler les trois pièces en un triangle
rectangle isocèle.
...
\hfill\strut
 ...
 Il y a intérêt à ce que les pièces d'un tel puzzle soient
indéformables --- donc des pièces standard --- afin que l'élève
ne risque pas de les déformer, fût-ce involontairement. C'est
ici que la fonctionnalité \texttt{Standardiser} va intervenir:
on commence par \hyperlink{chapmedit.Selectmul}{sélectionner}
simultanément les trois formes. On actionne ensuite l'item de
menu \textit{Fichiers.Standardiser}, ce qui fait apparaître une
fenêtre: \begin{center}
\includegraphics[width=0.4\linewidth]{Std.jpg}
\leg{\label{Stdjpg}} \end{center}
...
Il s'agit cette fois de sauvegarder la famille de formes standard
qui vient d'être constituée, soit dans un nouveau fichier
(d'extension \texttt{.std})\index{.std@\texttt{.std}} soit dans
un fichier déjà existant où il reste encore de la place, un
fichier pouvant contenir jusqu'à quatre familles. Dans les deux
cas, une troisième fenêtre s'ouvrira ensuite afin de choisir le
nom du fichier de sauvegarde. Après quoi la nouvelle famille de
formes standard sera disponible selon la procédure indiquée à la
section \ref{autresjeux}. Si l'utilisateur a attribué une
étiquette à une  forme ayant ainsi été \og standardisée\fg, c'est ce nom qui apparaîtra sur le
bouton de la fenêtre de construction
de la famille de formes standards.

\textbf{Remarque:} Après que des formes libres aient été
utilisées pour définir des formes standard, via l'opération qui
vient d'être décrite, ces formes libres restent affichées à
l'écran. De plus elle continuent d'être des formes libres. Pour
avoir accès aux formes standard destinées à les remplacer, il
convient de charger le nouveau fichier de formes standard, via le menu \textit{Préférences}.
...
l'extension \texttt{.std}\index{.std@\texttt{.std}}. Nous en décrivons ci-dessous la structure et la syntaxe.  Tout
...
\begin{center}
\includegraphics[width=0.7\linewidth]{rosaceastrica.eps}
\leg{: Une rosace \og astrica\fg\ (ASsemblage de TRIangles et de CArrés)}
\end{center}
\vfill
\strut
