...
Si le logiciel a été correctement installé, il est aussi possible d'ouvrir un fichier en le tirant dans une
fenêtre d'{\sffamily\bfseries Apprenti Géomètre} déjà ouverte. Et si {\sffamily\bfseries Apprenti Géomètre}
n'a pas été lancé, un double clic sur un fichier d'extension \texttt{.fag} doit normalement provoquer les ouvertures successives
d'{\sffamily\bfseries Apprenti Géomètre} et du fichier lui-même.
...
l'application ainsi que la langue de travail et certaines des préférences de l'utilisateur
concernant les formes géométriques (formes pointées, fléchées ou encore bifaces).
...
nécessitent la \textit{sélection}\index{Selectionner@Sélectionner} d'au moins une forme géométrique. Beaucoup
peuvent s'appliquer à plusieurs formes simultanément. Certaines s'appliquent nécessairement à
plusieurs formes simultanément. Ainsi les mouvements  s'appliquent automatiquement à
...
placer le curseur de la souris au-dessus de la forme à sélectionner; la couleur du bord passer au magenta;
...
\item Si la souris utilisée comporte une roulette, la pression sur la barre d'espacement peut être remplacée par une rotation d'un cran de la roulette.
...
\hyperlink{chapoutils.Plans}{\texttt{Arrière-plan}} permettent de modifier l'ordre initial.
 \end{enumerate}
